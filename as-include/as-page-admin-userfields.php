...
/*
	...
	File: as-include/as-page-admin-userfields.php
	...
	elseif (as_clicked('dosavefield')) {
		...
		if (!as_check_form_security_code('admin/userfields', as_post_text('code')))
			...
			if (as_post_text('dodelete')) {
				...
				$inflags=$intype | ($inonregister ? AS_FIELD_FLAGS_ON_REGISTER : 0);
	...
	$content['form']=array(
		...
		'fields' => array(
			...
			'onregister' => array(
				...
				'type' => 'checkbox',
				'value' => isset($inonregister) ? $inonregister : (@$editfield['flags']&AS_FIELD_FLAGS_ON_REGISTER),
			),
		),
