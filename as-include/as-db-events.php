...
/*
	...
	File: as-include/as-db-events.php
...
/*
	...
	as-db-favorites.php). Also handles truncation.
*/
	{
	...
	//	If this is for a question entity, check the shared event stream doesn't have too many entries for that question
		...
		if ($entitytype==AS_ENTITY_QUESTION) {
			$truncate=as_db_read_one_value(as_db_query_sub(
				...
				$entitytype, $entityid, $questionid, AS_DB_MAX_EVENTS_PER_Q
			), true);
...
*/
	{
		...
		as_db_query_sub(
			"INSERT INTO ^userevents (userid, entitytype, entityid, questionid, lastpostid, updatetype, lastuserid, updated) ".
			"VALUES ($, $, 0, #, #, $, $, ".$updatedsql.")",
			$userid, AS_ENTITY_NONE, $questionid, $lastpostid, $updatetype, $lastuserid
		);
	...
	//	First try truncating based on there being too many events for this question
		...
		if (isset($questionid)) {
			$truncate=as_db_read_one_value(as_db_query_sub(
				...
				$userid, $questionid, AS_DB_MAX_EVENTS_PER_Q
			), true);
