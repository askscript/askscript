...
/*
	...
	File: as-include/as-theme-base.php
...
*/
	...
	*/
	...
	/*
		...
		before main_parts(). See the docs for as_array_reorder() in as-util-sort.php for the other parameters.
	*/
		{
	...
	*/
	...
	//	The calling chain is initiated from as-page.php, or as-ajax-*.php for refreshing parts of a page, 
	//	For most HTML elements, the name of the function is similar to the element's CSS class, for example:
	//	search() outputs <div class="as-search">, q_list() outputs <div class="as-q-list">, etc...
		...
		function logged_in()
		{
			$this->output_split(@$this->content['loggedin'], 'as-logged-in', 'div');
		}
		...
		function main()
		{
			...
			$this->output('<div class="as-main'.(@$this->content['hidden'] ? ' as-main-hidden' : '').'">');
		...
		function favorite()
		{
			...
			if (isset($favorite)) {
				$this->output('<span class="as-favoriting" '.@$favorite['favorite_tags'].'>');
		...
		function main_part($key, $part)
		{
			...
			if ($partdiv)
				$this->output('<div class="as-part-'.strtr($key, '_', '-').'">'); // to help target CSS to page parts
	...
	/*
		...
		before any fields are output. See the docs for as_array_reorder() in as-util-sort.php for the other parameters.
	*/
		{
			require_once AS_INCLUDE_DIR.'as-util-sort.php';
	...
	/*
		...
		before any buttons are output. See the docs for as_array_reorder() in as-util-sort.php for the other parameters.
	*/
		{
			require_once AS_INCLUDE_DIR.'as-util-sort.php';
		...
		{
			$this->output('<select '.@$field['tags'].' class="as-form-'.$style.'-select">');
		...
		function ranking($ranking)
		{
			...
			$class=(@$ranking['type']=='users') ? 'as-top-users' : 'as-top-tags';
		...
		{
			if (isset($q_list['qs'])) {
				$this->output('<div class="as-q-list'.($this->list_vote_disabled($q_list['qs']) ? ' as-q-list-vote-disabled' : '').'">', '');
		...
		function voting($post)
		{
			if (isset($post['vote_view'])) {
				$this->output('<div class="as-voting '.(($post['vote_view']=='updown') ? 'as-voting-updown' : 'as-voting-net').'" '.@$post['vote_tags'].'>');
		...
		{
			$this->output('<div class="as-vote-buttons '.(($post['vote_view']=='updown') ? 'as-vote-buttons-updown' : 'as-vote-buttons-net').'">');

			switch (@$post['vote_state'])
			{
				case 'voted_up':
					$this->post_hover_button($post, 'vote_up_tags', '+', 'as-vote-one-button as-voted-up');
					break;
					
				case 'voted_up_disabled':
					$this->post_disabled_button($post, 'vote_up_tags', '+', 'as-vote-one-button as-vote-up');
					break;
					
				case 'voted_down':
					$this->post_hover_button($post, 'vote_down_tags', '&ndash;', 'as-vote-one-button as-voted-down');
					break;
					
				case 'voted_down_disabled':
					$this->post_disabled_button($post, 'vote_down_tags', '&ndash;', 'as-vote-one-button as-vote-down');
					break;
					
				case 'up_only':
					$this->post_hover_button($post, 'vote_up_tags', '+', 'as-vote-first-button as-vote-up');
					$this->post_disabled_button($post, 'vote_down_tags', '', 'as-vote-second-button as-vote-down');
					break;
				
				case 'enabled':
					$this->post_hover_button($post, 'vote_up_tags', '+', 'as-vote-first-button as-vote-up');
					$this->post_hover_button($post, 'vote_down_tags', '&ndash;', 'as-vote-second-button as-vote-down');
					break;

				default:
					$this->post_disabled_button($post, 'vote_up_tags', '', 'as-vote-first-button as-vote-up');
					$this->post_disabled_button($post, 'vote_down_tags', '', 'as-vote-second-button as-vote-down');
					break;
			}
		...
		function vote_count($post)
		{
			...
			$this->output('<div class="as-vote-count '.(($post['vote_view']=='updown') ? 'as-vote-count-updown' : 'as-vote-count-net').'"'.@$post['vote_count_tags'].'>');

			if ($post['vote_view']=='updown') {
				$this->output_split($post['upvotes_view'], 'as-upvote-count');
				$this->output_split($post['downvotes_view'], 'as-downvote-count');
			
			} else
				$this->output_split($post['netvotes_view'], 'as-netvote-count');
		...
		function q_view_content($q_view)
		{
			if (!empty($q_view['content'])) {
				$this->output('<div class="as-q-view-content">');
		...
		function a_list($a_list)
		{
			if (!empty($a_list)) {
				...
				$this->output('<div class="as-a-list'.($this->list_vote_disabled($a_list['as']) ? ' as-a-list-vote-disabled' : '').'" '.@$a_list['tags'].'>', '');
		...
		{
			$this->output('<div class="as-a-item-content">');
		...
		{
			$this->output('<div class="as-c-item-content">');
