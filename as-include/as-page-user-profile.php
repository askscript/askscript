...
/*
	...
	File: as-include/as-page-user-profile.php
...
//	Check the user exists and work out what can and can't be set (if not using single sign-on)
	...
	if (!AS_FINAL_EXTERNAL_USERS) { // if we're using integrated user management, we can know and show more
		require_once AS_INCLUDE_DIR.'as-app-messages.php';
		...
		$approvebutton=isset($maxlevelassign) && ($useraccount['level']<AS_USER_LEVEL_APPROVED) && ($maxlevelassign>=AS_USER_LEVEL_APPROVED) && (!($useraccount['flags'] & AS_USER_FLAGS_USER_BLOCKED)) && as_opt('moderate_users');
	...
	//	Show any extra privileges due to user's level or their points
		...
		foreach ($permitoptions as $permitoption)
			if ( // if not available to approved and email confirmed users with no points, but yes available to the user, it's something special
				as_permit_error($permitoption, $userid, AS_USER_LEVEL_APPROVED, AS_USER_FLAGS_EMAIL_CONFIRMED, 0) &&
	...
	//	Show other profile fields
		...
		foreach ($userfields as $userfield) {	
			if (($userfield['flags'] & AS_FIELD_FLAGS_LINK_URL) && !$fieldsediting)
				...
				$valuehtml=as_html($value, (($userfield['flags'] & AS_FIELD_FLAGS_MULTI_LINE) && !$fieldsediting) ? true : false);
			}
...
//	Information about user activity, available also with single sign-on integration
		...
		'fields' => array(
			...
			'points' => array(
				...
				'value' => (@$userpoints['points']==1)
					...
					: as_lang_html_sub('main/x_points', '<span class="as-uf-user-points">'.as_html(number_format(@$userpoints['points'])).'</span>'),
				'id' => 'points',
			),
			...
			'questions' => array(
				...
				'value' => '<span class="as-uf-user-q-posts">'.as_html(number_format(@$userpoints['qposts'])).'</span>',
				'id' => 'questions',
			),
			...
			'answers' => array(
				...
				'value' => '<span class="as-uf-user-a-posts">'.as_html(number_format(@$userpoints['aposts'])).'</span>',
				'id' => 'answers',
			),
		),
	);
	...
	if (as_opt('voting_on_qs') || as_opt('voting_on_as')) { // only show vote record if voting is enabled
		...
		if (as_opt('voting_on_qs')) {
			...
			$innervalue='<span class="as-uf-user-q-votes">'.number_format($qvotes).'</span>';
		...
		if (as_opt('voting_on_as')) {
			...
			$innervalue='<span class="as-uf-user-a-votes">'.number_format($avotes).'</span>';
		...
		$innervalue='<span class="as-uf-user-upvotes">'.number_format($upvotes).'</span>';
		...
		$innervalue='<span class="as-uf-user-upvoteds">'.number_format(@$userpoints['upvoteds']).'</span>';
		...
		$innervalue='<span class="as-uf-user-downvoteds">'.number_format(@$userpoints['downvoteds']).'</span>';
