...
/*
	...
	File: as-include/as-app-admin.php
...
*/
	{
		...
		$directory=@opendir(AS_LANG_DIR);
		if (is_resource($directory)) {
			while (($code=readdir($directory))!==false)
				if (is_dir(AS_LANG_DIR.$code) && isset($codetolanguage[$code]))
...
*/
	{
		if (file_exists(AS_INCLUDE_DIR.'as-db-install.php')) // file can be removed for extra security
			include_once AS_INCLUDE_DIR.'as-db-install.php';
