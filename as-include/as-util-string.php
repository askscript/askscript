...
/*
	...
	File: as-include/as-util-string.php
...
*/
	{
		...
		$separator=AS_PREG_INDEX_WORD_SEPARATOR;
		if ($splithyphens)
			$separator.='|\-';

		if ($delimiters) {
			if ($splitideographs)
				$separator.='|'.AS_PREG_CJK_IDEOGRAPHS_UTF8;
			...
			if ($splitideographs) // put spaces around CJK ideographs so they're treated as separate words
				$string=preg_replace('/'.AS_PREG_CJK_IDEOGRAPHS_UTF8.'/', ' \0 ', $string);
		}
...
*/
	{
		...
		return preg_split('/'.AS_PREG_BLOCK_WORD_SEPARATOR.'+/', $wordstring, -1, PREG_SPLIT_NO_EMPTY);
	}
...
*/
	{
		...
		foreach ($blockwords as $blockword) { // * in rule maps to [^ ]* in regular expression
			...
			if (!preg_match('/^('.AS_PREG_CJK_IDEOGRAPHS_UTF8.')/', $blockword))
...
*/
	{
		...
		if (strlen($wordspreg)) {
			...
			$string=preg_replace('/'.AS_PREG_BLOCK_WORD_SEPARATOR.'/', ' ', $string);
...
//	Some static definitions and initialization
	...
	@define('AS_PREG_INDEX_WORD_SEPARATOR', '[\n\r\t\ \!\"\\\'\(\)\*\+\,\.\/\:\;\<\=\>\?\[\\\\\]\^\`\{\|\}\~]');
		// Notable exclusions here: $ & - _ # % @
		
	@define('AS_PREG_BLOCK_WORD_SEPARATOR', '[\n\r\t\ \!\"\\\'\(\)\+\,\.\/\:\;\<\=\>\?\[\\\\\]\^\`\{\|\}\~\$\&\-\_\#\%\@]');
		// Asterisk (*) excluded here because it's used to match anything
		
	@define('AS_PREG_CJK_IDEOGRAPHS_UTF8', '\xE2[\xBA-\xBF][\x80-\xBF]|\xE3[\x80\x88-\xBF][\x80-\xBF]|[\xE4-\xE9][\x80-\xBF][\x80-\xBF]|\xEF[\xA4-\xAB][\x80-\xBF]|\xF0[\xA0-\xAF][\x80-\xBF][\x80-\xBF]');
