...
/*
	...
	File: as-include/as-page-admin-default.php
...
//	For non-text options, lists of option types, minima and maxima
	...
	$optionmaximum=array(
		...
		'page_size_ask_check_qs' => AS_DB_RETRIEVE_QS_AS,
		'page_size_ask_tags' => AS_DB_RETRIEVE_QS_AS,
...
//	Define the options to show (and some other visual stuff) based on request
	...
	switch ($adminsection) {
		...
		case 'feeds':
			...
			$showoptions=array('feed_for_questions', 'feed_for_as', 'feed_for_activity');
			...
			$checkboxtodisplay=array(
				'feed_per_category' => 'option_feed_for_as || option_feed_for_questions || option_feed_for_unanswered || option_feed_for_activity',
			);
			break;
				...
				case 'neat_urls':
					...
					foreach ($rawoptions as $rawoption)
						$neatoptions[$rawoption]=
							'<iframe src="'.as_path_html('url/test/'.AS_URL_TEST_STRING, array('dummy' => '', 'param' => AS_URL_TEST_STRING), null, $rawoption).'" width="20" height="16" style="vertical-align:middle; border:0" scrolling="no" frameborder="0"></iframe>&nbsp;'.
							'<small>'.
