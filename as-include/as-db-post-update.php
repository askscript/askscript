...
/*
	...
	File: as-include/as-db-post-update.php
...
*/
	...
	function as_db_post_set_selchildid($questionid, $selchildid, $lastuserid=null, $lastip=null)
/*
...
*/
	{
		...
		if (isset($selchildid) && isset($lastuserid) && isset($lastip))
			as_db_query_sub(
				...
				AS_UPDATE_SELECTED, $lastuserid, $lastip, $selchildid
			);
	}
	
	
	function as_db_post_set_closed($questionid, $closedbyid, $lastuserid=null, $lastip=null)
/*
...
*/
	{
		if (isset($lastuserid) || isset($lastip)) {
			as_db_query_sub(
				...
				$closedbyid, AS_UPDATE_CLOSED, $lastuserid, $lastip, $questionid
			);
		} else
			as_db_query_sub(
...
*/
	{
		if (isset($lastuserid) || isset($lastip))
			as_db_query_sub(
				...
				$parentid, AS_UPDATE_PARENT, $lastuserid, $lastip, $postid
			);
		else
			as_db_query_sub(
...
*/
	{
		if (isset($lastuserid) || isset($lastip))
			as_db_query_sub(
				...
				$categoryid, AS_UPDATE_CATEGORY, $lastuserid, $lastip, $postid
			);
		else
			as_db_query_sub(
...
*/
	{
		if (as_should_update_counts())
			as_db_query_sub("REPLACE ^options (title, content) SELECT 'cache_flaggedcount', COUNT(*) FROM ^posts WHERE flagcount>0 AND type IN ('Q', 'A', 'C')");
	}
