...
/*
	...
	File: as-include/as-db-selects.php
...
*/
	{
		...
		if (isset($voteuserid)) {
			...
			array_push($selectspec['arguments'], $voteuserid, $voteuserid, AS_ENTITY_QUESTION);
		}
...
*/
	{
		...
		array_push($selectspec['arguments'], AS_IGNORED_WORDS_FREQ, $questionid, AS_IGNORED_WORDS_FREQ, AS_IGNORED_WORDS_FREQ,
			$questionid, AS_IGNORED_WORDS_FREQ, AS_IGNORED_WORDS_FREQ, $questionid, AS_IGNORED_WORDS_FREQ, $questionid, $count);
...
*/
	{
		...
		$selectspec['source'].=" WHERE ^posts.userid=".(AS_FINAL_EXTERNAL_USERS ? "$" : "(SELECT userid FROM ^users WHERE handle=$ LIMIT 1)")." AND type='Q' ORDER BY ^posts.created DESC LIMIT #,#";
...
*/
	{
		...
		array_push($selectspec['arguments'], $userid, AS_ENTITY_QUESTION);
		$selectspec['sortdesc']='created';
...
*/
	{
		...
		return array(
			...
			'arguments' => array(AS_ENTITY_USER, AS_ENTITY_TAG, AS_ENTITY_CATEGORY, $userid, AS_ENTITY_QUESTION),
		);
	}	
...
/*
	...
	favorited and which no longer post to user streams (see long comment in as-db-favorites.php).
*/
	{
		require_once AS_INCLUDE_DIR.'as-app-updates.php';
