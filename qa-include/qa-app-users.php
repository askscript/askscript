	...
	*/
		{
			...
			setcookie('as_session', $handle.'/'.$sessioncode.'/'.($remember ? 1 : 0), $remember ? (time()+2592000) : 0, '/', QA_COOKIE_DOMAIN);
		}
	...
	*/
		{
			...
			setcookie('as_session', false, 0, '/', QA_COOKIE_DOMAIN);
		}
	...
	*/
		{
			...
			return strlen($handle) ? ('<a href="'.as_path_html('user/'.$handle).'" class="qa-user-link'
				.($favorited ? ' qa-user-favorited' : '').($microformats ? ' url nickname' : '').'">'.as_html($handle).'</a>') : '';
		}
	...
	*/	
		{
			...
			if (as_opt('avatar_allow_gravatar') && ($flags & QA_USER_FLAGS_SHOW_GRAVATAR))
				$html=as_get_gravatar_html($email, $size);
			elseif (as_opt('avatar_allow_upload') && (($flags & QA_USER_FLAGS_SHOW_AVATAR)) && isset($blobid))
			...
			return (isset($html) && strlen($handle)) ? ('<a href="'.as_path_html('user/'.$handle).'" class="qa-avatar-link">'.$html.'</a>') : $html;
		}
...
*/
	{
		return as_get_logged_in_user_field(QA_FINAL_EXTERNAL_USERS ? 'publicusername' : 'handle');
	}
...
*/
	{
		if (QA_FINAL_EXTERNAL_USERS)
			return as_get_logged_in_user_field('blocked') ? QA_USER_FLAGS_USER_BLOCKED : 0;
		else
...
*/
	{
		...
		if ((!$error) && isset($userid) && ($flags & QA_USER_FLAGS_MUST_CONFIRM) && as_opt('confirm_user_emails'))
			$error='confirm';
			
		if ((!$error) && isset($userid) && ($flags & QA_USER_FLAGS_MUST_APPROVE) && as_opt('moderate_users'))
			$error='approve';
...
*/
	{
		...
		$permit=isset($permitoption) ? as_opt($permitoption) : QA_PERMIT_ALL;
...
*/
	{
		...
		if ($userlevel < QA_USER_LEVEL_APPROVED) { // approved users and above aren't shown captchas
			...
			elseif (as_opt('moderate_users') && as_opt('captcha_on_unapproved'))
				$reason='approve';
			elseif (as_opt('confirm_user_emails') && as_opt('captcha_on_unconfirmed') && !(as_get_logged_in_flags() & QA_USER_FLAGS_EMAIL_CONFIRMED) )
				$reason='confirm';
		}
...
*/
	{
		...
		if (
			...
			if (isset($userid)) {
				if (as_opt('moderate_users') && as_opt('moderate_unapproved') && ($userlevel<QA_USER_LEVEL_APPROVED))
					$reason='approve';
				elseif (as_opt('confirm_user_emails') && as_opt('moderate_unconfirmed') && !(as_get_logged_in_flags() & QA_USER_FLAGS_EMAIL_CONFIRMED) )
					$reason='confirm';
...
*/
	{
		...
		if ( (!as_is_logged_in()) && !@$as_form_key_cookie_set) {
			...
			if (strlen(@$_COOKIE['as_key'])!=QA_FORM_KEY_LENGTH) {
				require_once QA_INCLUDE_DIR.'qa-util-string.php';
				$_COOKIE['as_key']=as_random_alphanum(QA_FORM_KEY_LENGTH);
			}
			
			setcookie('as_key', $_COOKIE['as_key'], time()+2*QA_FORM_EXPIRY_SECS, '/', QA_COOKIE_DOMAIN); // extend on every page request
		}
	}
