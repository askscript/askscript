...
//	Find the user profile and questions and answers for this handle
	...
	list($useraccount, $userprofile, $userfields, $usermessages, $userpoints, $userlevels, $navcategories, $userrank)=
		as_db_select_with_pending(
			...
			QA_FINAL_EXTERNAL_USERS ? null : as_db_user_profile_selectspec($handle, false),
			QA_FINAL_EXTERNAL_USERS ? null : as_db_userfields_selectspec(),
			QA_FINAL_EXTERNAL_USERS ? null : as_db_recent_messages_selectspec(null, null, $handle, false, as_opt_if_loaded('page_size_wall')),
			as_db_user_points_selectspec($identifier),
			as_db_user_levels_selectspec($identifier, QA_FINAL_EXTERNAL_USERS, true),
...
//	Check the user exists and work out what can and can't be set (if not using single sign-on)
	...
	if (!QA_FINAL_EXTERNAL_USERS) { // if we're using integrated user management, we can know and show more
		...
		$approvebutton=isset($maxlevelassign) && ($useraccount['level']<QA_USER_LEVEL_APPROVED) && ($maxlevelassign>=QA_USER_LEVEL_APPROVED) && (!($useraccount['flags'] & QA_USER_FLAGS_USER_BLOCKED)) && as_opt('moderate_users');
			...
			elseif (as_clicked('dosave')) {
				...
				if (!as_check_form_security_code('user-edit-'.$handle, as_post_text('code'))) {
					...
					if (as_post_text('removeavatar')) {
						as_db_user_set_flag($userid, QA_USER_FLAGS_SHOW_AVATAR, false);
					...
					if ($fieldseditable) {
						...
						if (!isset($errors['email']))
							if ($inemail != $useraccount['email']) {
								...
								as_db_user_set_flag($userid, QA_USER_FLAGS_EMAIL_CONFIRMED, false);
							}
		...
		if (as_clicked('doapprove') || as_clicked('doblock') || as_clicked('dounblock') || as_clicked('dohideall') || as_clicked('dodelete')) {
			if (!as_check_form_security_code('user-'.$handle, as_post_text('code')))
				...
				if ($approvebutton && as_clicked('doapprove')) {
					require_once QA_INCLUDE_DIR.'qa-app-users-edit.php';
					as_set_user_level($userid, $useraccount['handle'], QA_USER_LEVEL_APPROVED, $useraccount['level']);
					as_redirect(as_request());
				}
				...
				if (isset($maxlevelassign) && ($maxuserlevel<QA_USER_LEVEL_MODERATOR)) {
					...
					if (as_clicked('dohideall') && !as_user_permit_error('permit_hide_show')) {
						...
						foreach ($postids as $postid)
							as_post_set_hidden($postid, true, $loginuserid);
							
						as_redirect(as_request());
					}
					
					if (as_clicked('dodelete') && ($loginlevel>=QA_USER_LEVEL_ADMIN)) {
						require_once QA_INCLUDE_DIR.'qa-app-users-edit.php';
...
//	Process bonus setting button
	...
	if ( ($loginlevel>=QA_USER_LEVEL_ADMIN) && as_clicked('dosetbonus') ) {
...
//	Prepare content for theme
	...
	if (isset($loginuserid) && !QA_FINAL_EXTERNAL_USERS) {
		...
		$as_content['favorite']=as_favorite_form(QA_ENTITY_USER, $useraccount['userid'], $favorite,
			as_lang_sub($favorite ? 'main/remove_x_favorites' : 'users/add_user_x_favorites', $handle));
	}

	$as_content['script_rel'][]='qa-content/qa-user.js?'.QA_VERSION;
...
//	General information about the user, only available if we're using internal user management
		...
		$as_content['form_profile']=array(
			...
			'fields' => array(
				...
				'duration' => array(
					...
					'value' => as_html(as_time_to_string(as_opt('db_time')-$useraccount['created'])),
					'id' => 'duration',
				),
				...
				'level' => array(
					...
					'note' => (($useraccount['flags'] & QA_USER_FLAGS_USER_BLOCKED) && isset($maxlevelassign)) ? as_lang_html('users/user_blocked') : '',
					'id' => 'level',
				),
			),
		);
	...
	//	Private message link
		...
		if ( as_opt('allow_private_messages') && isset($loginuserid) && ($loginuserid!=$userid) && !($useraccount['flags'] & QA_USER_FLAGS_NO_MESSAGES) && !$userediting )
	...
	//	Show any extra privileges due to user's level or their points
		...
		foreach ($permitoptions as $permitoption)
			if ( // if not available to approved and email confirmed users with no points, but yes available to the user, it's something special
				as_permit_error($permitoption, $userid, QA_USER_LEVEL_APPROVED, QA_USER_FLAGS_EMAIL_CONFIRMED, 0) &&
	...
	//	Show email address only if we're an administrator
		...
		if (($loginlevel>=QA_USER_LEVEL_ADMIN) && !as_user_permit_error()) {
			$doconfirms=as_opt('confirm_user_emails') && ($useraccount['level']<QA_USER_LEVEL_EXPERT);
	...
	//	Show IP addresses and times for last login or write - only if we're a moderator or higher
		...
		if (($loginlevel>=QA_USER_LEVEL_MODERATOR) && !as_user_permit_error()) {
			$as_content['form_profile']['fields']['lastlogin']=array(
				...
				'value' =>
					strtr(as_lang_html('users/x_ago_from_y'), array(
						'^1' => as_time_to_string(as_opt('db_time')-$useraccount['loggedin']),
						'^2' => as_ip_anchor_html($useraccount['loginip']),
					)),
				...
				$as_content['form_profile']['fields']['lastwrite']=array(
					...
					'value' =>
						strtr(as_lang_html('users/x_ago_from_y'), array(
							...
							'^2' => as_ip_anchor_html($useraccount['writeip']),
						)),
					'note' => $userediting ? null : as_lang_html('users/only_shown_moderators'),
					'id' => 'lastwrite',
				);
	...
	//	Show other profile fields
		...
		foreach ($userfields as $userfield) {	
			if (($userfield['flags'] & QA_FIELD_FLAGS_LINK_URL) && !$fieldsediting)
				...
				$valuehtml=as_html($value, (($userfield['flags'] & QA_FIELD_FLAGS_MULTI_LINE) && !$fieldsediting) ? true : false);
			}
	...
	//	Edit form or button, if appropriate
		...
		if ($userediting) {
			...
			if (
				(as_opt('avatar_allow_gravatar') && ($useraccount['flags'] & QA_USER_FLAGS_SHOW_GRAVATAR)) ||
				(as_opt('avatar_allow_upload') && (($useraccount['flags'] & QA_USER_FLAGS_SHOW_AVATAR)) && isset($useraccount['avatarblobid']))
			) {
...
//	Information about user activity, available also with single sign-on integration
		...
		'fields' => array(
			...
			'points' => array(
				...
				'value' => (@$userpoints['points']==1)
					? as_lang_html_sub('main/1_point', '<span class="qa-uf-user-points">1</span>', '1')
					: as_lang_html_sub('main/x_points', '<span class="qa-uf-user-points">'.as_html(number_format(@$userpoints['points'])).'</span>'),
				'id' => 'points',
			),
			...
			'questions' => array(
				'type' => 'static',
				'label' => as_lang_html('profile/questions'),
				'value' => '<span class="qa-uf-user-q-posts">'.as_html(number_format(@$userpoints['qposts'])).'</span>',
				'id' => 'questions',
			),
			...
			'answers' => array(
				...
				'value' => '<span class="qa-uf-user-a-posts">'.as_html(number_format(@$userpoints['aposts'])).'</span>',
				'id' => 'answers',
			),
		),
	);
	...
	if (as_opt('comment_on_qs') || as_opt('comment_on_as')) { // only show comment count if comments are enabled
		$as_content['form_activity']['fields']['comments']=array(
			...
			'value' => '<span class="qa-uf-user-c-posts">'.as_html(number_format(@$userpoints['cposts'])).'</span>',
			'id' => 'comments',
		);
	}
	...
	if (as_opt('voting_on_qs') || as_opt('voting_on_as')) { // only show vote record if voting is enabled
		...
		if (as_opt('voting_on_qs')) {
			...
			$votedonvalue.=($qvotes==1) ? as_lang_html_sub('main/1_question', $innervalue, '1')
				: as_lang_html_sub('main/x_questions', $innervalue);
		...
		$votegotvalue=((@$userpoints['upvoteds']==1) ? as_lang_html_sub('profile/1_up_vote', $innervalue, '1')
	...
	if (@$userpoints['points'])
		$as_content['form_activity']['fields']['points']['value'].=
			as_lang_html_sub('profile/ranked_x', '<span class="qa-uf-user-rank">'.number_format($userrank).'</span>');
	
	if (@$userpoints['aselects'])
		$as_content['form_activity']['fields']['questions']['value'].=($userpoints['aselects']==1)
			? as_lang_html_sub('profile/1_with_best_chosen', '<span class="qa-uf-user-q-selects">1</span>', '1')
			: as_lang_html_sub('profile/x_with_best_chosen', '<span class="qa-uf-user-q-selects">'.number_format($userpoints['aselects']).'</span>');
	
	if (@$userpoints['aselecteds'])
		$as_content['form_activity']['fields']['answers']['value'].=($userpoints['aselecteds']==1)
			? as_lang_html_sub('profile/1_chosen_as_best', '<span class="qa-uf-user-a-selecteds">1</span>', '1')
			: as_lang_html_sub('profile/x_chosen_as_best', '<span class="qa-uf-user-a-selecteds">'.number_format($userpoints['aselecteds']).'</span>');
