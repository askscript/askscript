	...
	elseif (as_clicked('dosaveoptions') || as_clicked('doaddpage') || as_clicked('doaddlink')) {
		...
		if (!as_check_form_security_code('admin/pages', as_post_text('code')))
			...
			//	Verify the name (navigation link) is legitimate
				...
				elseif (as_strlen($inname)>QA_DB_MAX_CAT_PAGE_TITLE_LENGTH)
					$errors['name']=as_lang_sub('main/max_length_x', QA_DB_MAX_CAT_PAGE_TITLE_LENGTH);
				...
				//	Verify the url is legitimate (vaguely)
					...
					elseif (as_strlen($inurl)>QA_DB_MAX_CAT_PAGE_TAGS_LENGTH)
						$errors['url']=as_lang_sub('main/max_length_x', QA_DB_MAX_CAT_PAGE_TAGS_LENGTH);
	
				} else {
				
				//	Verify the heading is legitimate
				
					if (as_strlen($inheading)>QA_DB_MAX_TITLE_LENGTH)
						$errors['heading']=as_lang_sub('main/max_length_x', QA_DB_MAX_TITLE_LENGTH);
				...
				//	Verify the slug is legitimate (and try some defaults if we're creating a new page, and it's not)
					...
					for ($attempt=0; $attempt<100; $attempt++) {
						...
						elseif (as_strlen($inslug)>QA_DB_MAX_CAT_PAGE_TAGS_LENGTH)
							$errors['slug']=as_lang_sub('main/max_length_x', QA_DB_MAX_CAT_PAGE_TAGS_LENGTH);
			...
			//	Perform appropriate database action
				...
				if (isset($editpage['pageid'])) { // changing existing page
					...
					if (empty($errors)) {
						if ($isexternal)
							$pageid=as_db_page_create($inname, QA_PAGE_FLAGS_EXTERNAL | ($innewwindow ? QA_PAGE_FLAGS_NEW_WINDOW : 0), $inurl, null, null, $inpermit);
						else {
...
//	Prepare content for theme
	...
	//	List of custom pages or links
		...
		foreach ($pages as $page) {
			...
			$listhtml.=strtr(as_lang_html(($page['flags'] & QA_PAGE_FLAGS_EXTERNAL) ? 'admin/edit_link' : 'admin/edit_page'), array(
