...
*/
	{
		...
		$thisqa=as_version_to_float(QA_VERSION);
...
*/
	{
		...
		if (!file_exists(QA_BASE_DIR.'qa-config.php'))
			as_fatal_error('The config file could not be found. Please read the instructions in qa-config-example.php.');
		
		require_once QA_BASE_DIR.'qa-config.php';
		
		$as_request_map=is_array(@$QA_CONST_PATH_MAP) ? $QA_CONST_PATH_MAP : array();
		...
		if (defined('QA_WORDPRESS_INTEGRATE_PATH') && strlen(QA_WORDPRESS_INTEGRATE_PATH)) {
			...
			if (!is_readable(QA_WORDPRESS_LOAD_FILE))
				as_fatal_error('Could not find wp-load.php file for WordPress integration - please check QA_WORDPRESS_INTEGRATE_PATH in qa-config.php');
		}
	}
...
*/
	...
	//	Possible URL schemes for Q2A and the string used for url scheme testing
		...
		define('QA_URL_FORMAT_PARAMS', 4); // http://...../?qa=123&as_1=why-is-the-sky-blue
		define('QA_URL_FORMAT_SAFEST', 5); // http://...../index.php?qa=123&as_1=why-is-the-sky-blue
...
*/
	{
		as_register_module('filter', 'qa-filter-basic.php', 'as_filter_basic', '');
		as_register_module('editor', 'qa-editor-basic.php', 'as_editor_basic', '');
		as_register_module('viewer', 'qa-viewer-basic.php', 'as_viewer_basic', '');
		as_register_module('event', 'qa-event-limits.php', 'as_event_limits', 'Q2A Event Limits');
		as_register_module('event', 'qa-event-notify.php', 'as_event_notify', 'Q2A Event Notify');
		as_register_module('event', 'qa-event-updates.php', 'as_event_updates', 'Q2A Event Updates');
		as_register_module('search', 'qa-search-basic.php', 'as_search_basic', '');
		as_register_module('widget', 'qa-widget-activity-count.php', 'as_activity_count', 'Activity Count');
		as_register_module('widget', 'qa-widget-ask-box.php', 'as_ask_box', 'Ask Box');
		as_register_module('widget', 'qa-widget-related-qs.php', 'as_related_qs', 'Related Questions');
	}
...
*/
	{
		...
		foreach ($pluginfiles as $pluginfile)
			if (file_exists($pluginfile)) {
				...
				$as_plugin_urltoroot=substr($as_plugin_directory, strlen(QA_BASE_DIR));
...
//	Functions for registering different varieties of Q2A modularity
	...
	function as_register_module($type, $include, $class, $name, $directory=QA_INCLUDE_DIR, $urltoroot=null)
/*
...
*/
	...
	function as_register_layer($include, $name, $directory=QA_INCLUDE_DIR, $urltoroot=null)
/*
...
*/
	...
	function as_register_overrides($include, $directory=QA_INCLUDE_DIR, $urltoroot=null)
/*
...
*/
	{
		...
		if (empty($as_plugin_directory) || empty($as_plugin_urltoroot))
			as_fatal_error('as_register_plugin_module() can only be called from a plugin qa-plugin.php file');
...
*/
	{
		...
		if (empty($as_plugin_directory) || empty($as_plugin_urltoroot))
			as_fatal_error('as_register_plugin_layer() can only be called from a plugin qa-plugin.php file');
...
*/
	{
		...
		if (empty($as_plugin_directory) || empty($as_plugin_urltoroot))
			as_fatal_error('as_register_plugin_overrides() can only be called from a plugin qa-plugin.php file');
...
*/
	{
		...
		if (empty($as_plugin_directory) || empty($as_plugin_urltoroot))
			as_fatal_error('as_register_plugin_phrases() can only be called from a plugin qa-plugin.php file');
