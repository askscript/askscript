	...
	if (as_clicked('dosaveprofile') && !$isblocked) {
		...
		if (!as_check_form_security_code('account', as_post_text('code')))
			...
			if (as_opt('allow_private_messages'))
				as_db_user_set_flag($userid, QA_USER_FLAGS_NO_MESSAGES, !$inmessages);
			
			if (as_opt('allow_user_walls'))
				as_db_user_set_flag($userid, QA_USER_FLAGS_NO_WALL_POSTS, !$inwallposts);
			
			if (as_opt('mailing_enabled'))
				as_db_user_set_flag($userid, QA_USER_FLAGS_NO_MAILINGS, !$inmailings);
			
			as_db_user_set_flag($userid, QA_USER_FLAGS_SHOW_AVATAR, ($inavatar=='uploaded'));
			as_db_user_set_flag($userid, QA_USER_FLAGS_SHOW_GRAVATAR, ($inavatar=='gravatar'));
			...
			if (is_array(@$_FILES['file']) && $_FILES['file']['size']) {
				...
				switch (as_user_permit_error(null, QA_LIMIT_UPLOADS))
				{
					...
					case false:
						as_limits_increment($userid, QA_LIMIT_UPLOADS);
