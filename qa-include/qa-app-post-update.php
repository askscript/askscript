...
*/
	{
		as_question_set_status($oldquestion, $hidden ? QA_POST_STATUS_HIDDEN : QA_POST_STATUS_NORMAL, $userid, $handle, $cookieid, $answers, $commentsfollows, $closepost);
	}
...
*/
	{
		...
		as_db_post_set_type($oldquestion['postid'], $newtype, $setupdated ? $userid : null, $setupdated ? as_remote_ip_address() : null, QA_UPDATE_VISIBLE);
		
		if ( $wasqueued && ($status==QA_POST_STATUS_NORMAL) && as_opt('moderate_update_time') ) { // ... for approval of a post, can set time to now instead			
...
*/
	{
		...
		as_db_post_set_content($oldanswer['postid'], $oldanswer['title'], $content, $format, $oldanswer['tags'], $notify,
			$setupdated ? $userid : null, $setupdated ? as_remote_ip_address() : null, QA_UPDATE_CONTENT, $name);
...
*/
	{
		as_answer_set_status($oldanswer, $hidden ? QA_POST_STATUS_HIDDEN : QA_POST_STATUS_NORMAL, $userid, $handle, $cookieid, $question, $commentsfollows);
	}
...
*/
	{
		...
		as_db_post_set_type($oldanswer['postid'], $newtype, $setupdated ? $userid : null, $setupdated ? as_remote_ip_address() : null, QA_UPDATE_VISIBLE);
...
*/
	{
		...
		as_db_post_set_type($oldanswer['postid'], $newtype, ($wasqueued || $silent) ? null : $userid,
			($wasqueued || $silent) ? null : as_remote_ip_address(), QA_UPDATE_TYPE);
...
*/
	{
		as_comment_set_status($oldcomment, $hidden ? QA_POST_STATUS_HIDDEN : QA_POST_STATUS_NORMAL, $userid, $handle, $cookieid, $question, $parent);
	}
...
*/
	{
		...
		as_db_post_set_type($oldcomment['postid'], $newtype, $setupdated ? $userid : null, $setupdated ? as_remote_ip_address() : null, QA_UPDATE_VISIBLE);
