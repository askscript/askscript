...
//	Check the user exists and work out what can and can't be set (if not using single sign-on)
	...
	if ( (!as_opt('allow_private_messages')) || (!is_array($toaccount)) || ($toaccount['flags'] & QA_USER_FLAGS_NO_MESSAGES) )
...
//	Check that we have permission and haven't reached the limit
	...
	switch (as_user_permit_error(null, QA_LIMIT_MESSAGES)) {
		case 'limit':
	...
	if (as_post_text('domessage')) {
		...
		if (!as_check_form_security_code('message-'.$handle, as_post_text('code')))
			...
			if (empty($errors)) {
				...
				$canreply=!(as_get_logged_in_flags() & QA_USER_FLAGS_NO_MESSAGES);
