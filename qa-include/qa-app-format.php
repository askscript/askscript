...
*/
	{
		...
		return '<a href="'.as_path_html('tag/'.$tag).'"'.($microformats ? ' rel="tag"' : '').' class="qa-tag-link'.
			($favorited ? ' qa-tag-favorited' : '').'">'.as_html($tag).'</a>';
	}
...
*/
	{
		...
		return '<a href="'.as_path_html('ip/'.$ip).'" title="'.as_lang_html_sub('main/ip_address_x', as_html($ip)).'" class="qa-ip-link">'.$anchorhtml.'</a>';
	}
...
/*
	...
	$options is an array which sets what is displayed (see as_post_html_defaults() in qa-app-options.php)
		...
		if ($isquestion) {
			...
			if (@$options['categoryview'] && isset($post['categoryname']) && isset($post['categorybackpath'])) {
				...
				$fields['where']=as_lang_html_sub_split('main/in_category_x',
					...
					'" class="qa-category-link'.$favoriteclass.'">'.as_html($post['categoryname']).'</a>');
			}
		}
...
/*
	...
	Pass viewing options in $options (see as_message_html_defaults() in qa-app-options.php)
*/
	{
		require_once QA_INCLUDE_DIR.'qa-app-users.php';
...
*/
	{
		if ((!QA_FINAL_EXTERNAL_USERS) && (as_get_logged_in_level()>=QA_USER_LEVEL_MODERATOR)) {
			return array(
				'users$' => array(
...
*/
	{
		if (
			...
			$navigation[($page['flags'] & QA_PAGE_FLAGS_EXTERNAL) ? ('custom-'.$page['pageid']) : ($page['tags'].'$')]=array(
				...
				'selected' => ($page['flags'] & QA_PAGE_FLAGS_EXTERNAL) && ( ($url==as_path(as_request())) || ($url==as_self_html()) ),
			);
		}
	}
...
*/
	{
		$template='<a href="#" class="qa-tag-link" onclick="return as_tag_click(this);">^</a>';

		$as_content['script_rel'][]='qa-content/qa-ask.js?'.QA_VERSION;
...
*/
	...
	//	Then load the selected theme if valid, otherwise load the Classic theme
		...
		$themeroothtml=as_html(as_path_to_root().'qa-theme/'.$theme.'/');
		...
		foreach ($loadlayers as $layer) {
			...
			if (strlen($layerphp)) {
				...
				$searchwordreplace=array(
					...
					'QA_HTML_THEME_LAYER_URLTOROOT' => "'".as_path_to_root().$layer['urltoroot']."'",
				);
...
*/
	{
		...
		if (strlen($blobid) && ($size>0)) {
			...
			$html='<img src="'.as_path_html('image', array('as_blobid' => $blobid, 'as_size' => $size), null, QA_URL_FORMAT_PARAMS).
