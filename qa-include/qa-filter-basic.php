...
*/
	...
	class as_filter_basic {
		...
		function filter_email(&$email, $olduser)
		{
			...
			if (as_strlen($email)>QA_DB_MAX_EMAIL_LENGTH)
				return as_lang_sub('main/max_length_x', QA_DB_MAX_EMAIL_LENGTH);
		}
		...
		function filter_handle(&$handle, $olduser)
		{
			...
			if (as_strlen($handle)>QA_DB_MAX_HANDLE_LENGTH)
				return as_lang_sub('main/max_length_x', QA_DB_MAX_HANDLE_LENGTH);
		}
		...
		function filter_question(&$question, &$errors, $oldquestion)
		{
			$this->validate_length($errors, 'title', @$question['title'], as_opt('min_len_q_title'),
				max(as_opt('min_len_q_title'), min(as_opt('max_len_q_title'), QA_DB_MAX_TITLE_LENGTH)));
			...
			if (isset($question['tags'])) {
				...
				elseif ($counttags>as_opt('max_num_q_tags'))
					...
					$this->validate_length($errors, 'tags', as_tags_to_tagstring($question['tags']), 0, QA_DB_MAX_TAGS_LENGTH); // for storage
			}
