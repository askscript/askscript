...
//	Get information about this question
	...
	list($question, $childposts, $achildposts, $parentquestion, $closepost, $extravalue, $categories, $favorite)=as_db_select_with_pending(
		...
		isset($userid) ? as_db_is_favorite_selectspec($userid, QA_ENTITY_QUESTION, $questionid) : null
	);
...
//	Prepare content for theme
	...
	if (isset($userid) && !$formrequested)
		$as_content['favorite']=as_favorite_form(QA_ENTITY_QUESTION, $questionid, $favorite, 
			as_lang($favorite ? 'question/remove_q_favorites' : 'question/add_q_favorites'));

	$as_content['script_rel'][]='qa-content/qa-question.js?'.QA_VERSION;

	if (isset($pageerror))
		$as_content['error']=$pageerror; // might also show voting error set in qa-index.php
