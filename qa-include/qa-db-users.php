...
*/
	...
	function as_db_user_create($email, $password, $handle, $level, $ip)
/*
...
*/
	{
		...
		as_db_query_sub(
			...
			$ip, $email, $salt, isset($password) ? as_db_calc_passcheck($password, $salt) : null, (int)$level, $handle, $ip
		);
...
*/
	{
		...
		as_db_query_sub('DELETE FROM ^userlogins WHERE userid=$', $userid);
		...
		as_db_query_sub('DELETE FROM ^uservotes WHERE userid=$', $userid);
...
*/
	...
	function as_db_user_set($userid, $field, $value)
/*
	Set $field of $userid to $value in the database users table
*/
	{
		as_db_query_sub(
			'UPDATE ^users SET '.as_db_escape_string($field).'=$ WHERE userid=$',
			$value, $userid
		);
	}
...
*/
	...
	function as_db_user_login_add($userid, $source, $identifier)
/*
...
*/
	...
	function as_db_user_login_find($source, $identifier)
/*
...
*/
	...
	function as_db_user_login_sync($sync)
/*
...
*/
	{
		if ( as_should_update_counts() && !QA_FINAL_EXTERNAL_USERS )
			as_db_query_sub(
